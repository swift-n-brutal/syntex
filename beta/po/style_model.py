...
from tensorpack import (InstanceNorm, BatchNorm, Conv2D, Conv2DTranspose,
...
def res_block(name, x, chan, pad_type, norm_type, alpha=0.2, bottleneck=False, pre_act=False, gain=1./np.sqrt(2.)):
    ...
    with tf.variable_scope(name):
        ...
        shortcut = x
        if pre_act:
            conv_branch = act("act_input", x, norm_type, alpha)
        else:
        ...
        if not pre_act:
            conv_branch = act("act_output", conv_branch, norm_type, None)
...
class StylePO(SynTexModelDesc):
    ...
    def __init__(self, args):
        super(StylePO, self).__init__(args)
    ...
    def get_parser(ap=None):
        ap = super(StylePO, StylePO).get_parser(ap)
        ...
        ap.add("--n-stage", type=int, default=5, help="This argument is fixed to 5.")
    ...
    def build_stage_preparation(self, x, gram_target, coefs, name="prep"):
        ...
        # However the loss of the last layer contributes to the calculation
        ...
        with tf.name_scope(name):
            ...
            for k in layers:
                ...
                loss_per_layer[k] = tf.reduce_mean(tf.square(gram - gram_target[k]),
    ...
    def build_stage(self, x, gram_target, coefs, name="stage"):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            ...
            with argscope([Conv2D, Conv2DTranspose], activation=acti, use_bias=False):
                for layer in reversed(feat):
                    if layer in grad_per_layer:
                        ...
                        with tf.variable_scope(layer):
                            ...
                            # add relu gate here
                            if self._gate:
                                gate = get_relu_gate("gate", feat[StylePO.GATE_SOURCE[layer]], 0.)
                            ...
                            if self._same_block:
                                ...
                                n_block = self._n_block * StylePO.N_BLOCK_BASE[layer]
                ...
                # output
                delta_x = pad_conv2d("conv_last", delta, 3, 1,
                    self._pad_type, activation=tf.identity, use_bias=True)
            if self._stop_grad:
    ...
    def build_graph(self, x, image_target):
        ...
        if ctx is not None and ctx.is_main_training_tower:
            ...
            add_moving_summary(self.loss, *self.loss_per_stage, *self.loss_per_layer_output.values())
...
def train(args):    
    ...
    SynTexTrainer(data, StylePO(args), n_gpu).train_with_defaults(
        callbacks=[
...
if __name__ == "__main__":
    ap = StylePO.get_parser()
    ap.add("--data-folder", type=str, default="../images/scaly")
    ap.add("--save-folder", type=str, default="train_log/impr")
